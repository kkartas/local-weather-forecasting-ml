...
class RecurrentRegressor(nn.Module):
    ...
    hidden state is projected to one scalar forecast target.
    """

    def __init__(self, input_size: int, cell: str = "lstm", hidden_size: int = 64, num_layers: int = 1):
        super().__init__()
        if cell == "lstm":
...
class TemporalBlock(nn.Module):
    """Simple causal temporal convolution block."""
...
class TCNRegressor(nn.Module):
    ...
    def __init__(self, input_size: int, channels: int = 64):
        ...
        self.blocks = nn.Sequential(TemporalBlock(channels, 1), TemporalBlock(channels, 2), TemporalBlock(channels, 4))
...
def make_dl_model(name: str, input_size: int) -> nn.Module:
    ...
    if name == "tcn":
        return TCNRegressor(input_size=input_size)
