...
- Values greater than `1` train each horizon in its own worker process.
  ...
  The shipped full-run configs use `1` worker because the MetDataPy scaler
  currently applies to a wide float64 feature frame inside each worker;
  multiple workers can exceed RAM on the target host even when CPU threads
  are capped.
